...
def lossFun(inputs, targets, hprev):
  ...
  for t in xrange(len(inputs)):
    ...
    loss += -np.log(ps[t][targets,0]) # softmax (cross-entropy loss)
...
while n < 200:
  ...
  inputs = data[np.random.random_integers(0,len(data) -1)].strip()
  inputs = [char_to_ix[i] for i in inputs]
  targets = [int(ch) for ch in result]

  # forward seq_length characters through the net and fetch gradient
  loss, dWxh, dWhh, dWhy, dbh, dby, hprev = lossFun(inputs, targets, hprev)
  smooth_loss = smooth_loss * 0.999 + loss * 0.001
  if n % 100 == 0: print 'iter %d, loss: %f' % (n, smooth_loss[0]) # print progress
